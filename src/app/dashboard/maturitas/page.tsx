...
// --- Conversion table from Konversi_MMR.xlsx ---
...
// --- Building block constraint: compute max score allowed for row at index ---
// If ANY previous row has skor 0, all subsequent rows max = 0 (frozen at 0)
// If ANY previous row has skor 1 (and none is 0 yet), all subsequent rows max = 1
function getMaxAllowed(rows: RowState[], idx: number): 0 | 1 | 2 {
  for (let i = 0; i < idx; i++) {
    const s = rows[i].skor
    if (s === 0) return 0   // "Tidak" (0) found earlier → all after must be 0
    if (s === 1) return 1   // "Sebagian" (1) found earlier → all after max 1
  }
  return 2 // no restriction
}
...
// --- Score Select with building-block maxAllowed enforcement ---
function SkorSelect({
  value, onChange, maxAllowed, isLocked,
}: {
  value: 0 | 1 | 2 | null
  ...
  // If locked completely (2 options only, or only 0 allowed)
  const availableOptions: (0 | 1 | 2 | null)[] = [null]
  for (let v = 0; v <= maxAllowed; v++) availableOptions.push(v as 0 | 1 | 2)
...
export default function MaturitasPage() {
  const [rows, setRows] = useState<RowState[]>(
    MMR_ITEMS.map(() => ({ dokumen: null, wawancara: null, observasi: null, skor: null }))
  )

  const totalSkor = useMemo(
    () => rows.reduce((sum, r) => sum + (r.skor ?? 0), 0),
    [rows]
  )
  ...
  // Skor max yang bisa diraih dengan kondisi building block
  const maxAchievable = useMemo(() => {
    let max = 0
    for (let i = 0; i < rows.length; i++) {
      const cap = getMaxAllowed(rows, i)
      // If row is already filled, use its value, else assume cap
      const effective = rows[i].skor !== null ? rows[i].skor! : cap
      max += effective
    }
    return Math.min(max, MMR_ITEMS.length * 2)
  }, [rows])

  function updateRow(idx: number, field: keyof RowState, value: TriState | 0 | 1 | 2 | null) {
    setRows((prev) => {
      const next = [...prev]
      // If skor is being updated, enforce building block cap
      if (field === 'skor') {
        const maxAllowed = getMaxAllowed(next, idx)
        const newSkor = value as (0 | 1 | 2 | null)
        if (newSkor !== null && newSkor > maxAllowed) return prev // reject illegal value
        next[idx] = { ...next[idx], skor: newSkor }

        // Cascade: fix subsequent rows that now violate the new cap
  ...
  function resetAll() {
    setRows(MMR_ITEMS.map(() => ({ dokumen: null, wawancara: null, observasi: null, skor: null })))
  }

  // Find first row with skor = 0 or skor = 1 to show warning
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">Penilaian Tingkat Maturitas MR — klik kolom untuk mengisi.</p>
        </div>
        <button
          onClick={resetAll}
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-3 text-sm text-amber-800 leading-relaxed">
        <span className="font-semibold">Sistem Penilaian Building Block:</span> Penilaian menggunakan sistem bertahap. Jika suatu pernyataan dinilai{' '}
        ...
        Kolom yang terkunci akan ditandai dengan ikon 🔒.
      </div>
      ...
      {firstZeroIdx >= 0 && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-5 py-2.5 text-sm text-red-700 flex items-center gap-2">
          ...
          <span>Pernyataan <strong>#{firstZeroIdx + 1}</strong> dinilai <strong>0</strong> — pernyataan #{firstZeroIdx + 2} s.d. #{MMR_ITEMS.length} dikunci pada nilai maks. 0. Skor maksimal yang bisa diperoleh: <strong>{rows.slice(0, firstZeroIdx + 1).reduce((s, r) => s + (r.skor ?? 0), 0)} dari {MMR_ITEMS.length * 2}</strong>.</span>
        </div>
      )}
      {firstZeroIdx < 0 && firstOneIdx >= 0 && (
        <div className="rounded-xl border border-yellow-200 bg-yellow-50 px-5 py-2.5 text-sm text-yellow-800 flex items-center gap-2">
          <span>⚠️</span>
          <span>Pernyataan <strong>#{firstOneIdx + 1}</strong> dinilai <strong>1</strong> — pernyataan #{firstOneIdx + 2} s.d. #{MMR_ITEMS.length} dibatasi maks. 1.</span>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {MMR_ITEMS.map((uraian, idx) => {
                ...
                const isLocked = maxAllowed < 2 // row has some constraint
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <TriStateSelect
                        value={row.dokumen}
                        onChange={(v) => updateRow(idx, 'dokumen', v)}
                    ...
                    <td className="px-3 py-3">
                      <TriStateSelect
                        value={row.wawancara}
                        onChange={(v) => updateRow(idx, 'wawancara', v)}
                        disabled={isFullyLocked}
                      />
                    </td>
                    <td className="px-3 py-3">
                      <TriStateSelect
                        value={row.observasi}
                        onChange={(v) => updateRow(idx, 'observasi', v)}
                        disabled={isFullyLocked}
                      />
                    </td>
                    <td className="px-3 py-3">
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h3 className={cn('text-xl font-bold font-serif', level.color)}>
                {level.label}
              </h3>
              <span className={cn(
