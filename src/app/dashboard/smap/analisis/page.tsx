...
import SmapAnalisisTable from './SmapAnalisisTable'

export default async function SmapAnalisisPage({
  searchParams,
}: {
  ...
  const { data: analisisList } = risikoIds.length > 0
    ? await supabase.from('smap_analisis').select('*').in('risiko_id', risikoIds)
    : { data: [] }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        <a
          href={`/dashboard/smap/identifikasi?konteks=${konteksId}`}
        ...
        <div className="flex-1">
          <h2 className="text-2xl font-bold tracking-tight font-serif">Analisis Risiko Penyuapan</h2>
          <p className="text-muted-foreground text-sm mt-0.5">
            Form 2 · Tahun <strong>{konteks.tahun}</strong> — <strong>{namaUnit}</strong>
          </p>
        </div>
      </div>
