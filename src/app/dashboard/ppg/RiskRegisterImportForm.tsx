...
import { importPpgRiskRegister, initialRiskImportState } from './risk-import-actions'
...
export function RiskRegisterImportForm({ defaultMode = 'bootstrap_library', allowModeChoice = true }: { defaultMode?: 'bootstrap_library' | 'operasional_assessment'; allowModeChoice?: boolean }) {
  ...
  return <section className="rounded-2xl border border-indigo-200 bg-white p-5 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="flex gap-3"><span className="rounded-xl bg-indigo-50 p-2.5 text-indigo-700"><FileSpreadsheet className="h-5 w-5" /></span><div><h3 className="font-bold text-slate-900">Impor Risk Register baku</h3><p className="mt-1 max-w-2xl text-sm text-slate-600">Gunakan sheet <b>Risk Register 2026</b>. Data masuk ke staging untuk dicocokkan dan dikurasi; tidak langsung mengubah Risk Library.</p></div></div>
      <a href="/templates/Template_RiskLibrary_Sektor_Publik_Redesign_.xlsx" download className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50"><Download className="h-4 w-4" />Unduh template baku</a>
    </div>
    ...
    <p className="mt-3 text-xs text-slate-500">Kolom yang tidak tersedia pada template—termasuk residual risk dan treated risk—tetap kosong dan harus dilengkapi dalam aplikasi.</p>
