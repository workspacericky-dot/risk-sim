...
import { getPpgAiRecommendationWorkspace, getPpgAnalytics, getPpgLedAnalytics, getPpgNationalRiskInsights } from '@/lib/ppg/data'
...
export default async function TitikRawanPage({ searchParams }: Props) {
  ...
  const [led, nationalRisks, ai] = await Promise.all([getPpgLedAnalytics(data.period.baselineStart, data.period.end), getPpgNationalRiskInsights(data.period.baselineStart, data.period.end), getPpgAiRecommendationWorkspace(data.period.year, data.period.quarter)])
  ...
  return <div className="space-y-6">
    ...
    {!summary.itemCount ? <EmptyState title="Belum ada data pada periode ini" description="Pilih periode lain atau impor worksheet melalui Referensi & Impor. Identitas pribadi tidak disimpan oleh modul PPG." /> : <>
      ...
      <InsightVisuals rows={assistedInsights} />
      <AiRecommendationPanel run={ai.run} candidates={ai.candidates} year={period.year} quarter={period.quarter} isDemo={ai.access.isDemo} error={ai.error} />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-bold text-slate-900">Insight B nasional per risiko generik</h3><p className="mt-1 text-xs text-slate-500">Persentase memakai populasi Satker terukur, bukan seluruh master Satker: metrik kejadian memakai Satker yang memiliki register/event pada risiko dan periode tersebut; metrik selera memakai Satker yang telah menetapkan selera terkait.</p>{nationalRisks.error ? <p className="mt-3 text-sm text-amber-700">Insight B belum tersedia: {nationalRisks.error}</p> : <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-xs"><thead className="text-slate-500"><tr><th className="p-2">Risiko generik</th><th className="p-2">Satker terdampak</th><th className="p-2">Di atas selera</th><th className="p-2">Upper limit</th><th className="p-2">Dampak tinggi</th><th className="p-2">Berulang</th><th className="p-2">Kontrol gagal</th><th className="p-2">Klaster 1/2/3</th><th className="p-2">Landasan program</th><th className="p-2">Keyakinan data</th></tr></thead><tbody>{nationalRisks.rows.filter((row) => row.affected_satkers > 0 || row.above_appetite_satkers > 0).map((row) => <tr key={row.risk_library_id} className="border-t border-slate-100"><td className="p-2"><b>{row.kode}</b><p className="mt-1 max-w-md text-slate-500">{row.peristiwa}</p></td><td className="p-2"><b className="text-indigo-700">{row.affected_pct.toFixed(1)}%</b><p className="mt-1 text-[10px] text-slate-400">{row.affected_satkers}/{row.eligible_satkers} Satker terukur</p></td><td className="p-2"><b className="text-amber-700">{row.above_appetite_pct.toFixed(1)}%</b><p className="mt-1 text-[10px] text-slate-400">{row.above_appetite_satkers}/{row.appetite_set_satkers} Satker terukur</p></td><td className="p-2 font-semibold text-rose-700">{row.upper_limit_satkers} Satker</td><td className="p-2">{row.high_impact_pct.toFixed(1)}%<p className="mt-1 text-[10px] text-slate-400">{row.high_impact_satkers}/{row.eligible_satkers}</p></td><td className="p-2">{row.recurring_pct.toFixed(1)}%<p className="mt-1 text-[10px] text-slate-400">{row.recurring_satkers}/{row.eligible_satkers}</p></td><td className="p-2">{row.control_failure_pct.toFixed(1)}%<p className="mt-1 text-[10px] text-slate-400">{row.control_failure_satkers}/{row.eligible_satkers}</p></td><td className="p-2">{row.cluster_1_satkers} / {row.cluster_2_satkers} / {row.cluster_3_satkers}</td><td className="p-2"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${row.recommended_for_program ? 'bg-rose-50 text-rose-700' : 'bg-slate-100 text-slate-600'}`}>{row.recommended_for_program ? 'PERTIMBANGKAN PPG' : 'MONITOR'}</span></td><td className="p-2 capitalize">{row.data_confidence}</td></tr>)}</tbody></table>{!nationalRisks.rows.some((row) => row.affected_satkers > 0 || row.above_appetite_satkers > 0) && <p className="p-4 text-sm text-slate-500">Belum ada loss event tervalidasi atau residual risk di atas selera pada periode analisis.</p>}</div>}</section>
