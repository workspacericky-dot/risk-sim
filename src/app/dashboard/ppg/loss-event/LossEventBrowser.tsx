...
function EventRow({ event, isPusat, riskOptions }: { event: Record<string, unknown>; isPusat: boolean; riskOptions: Record<string, unknown>[] }) {
  ...
  const failedControls = arrayRecords(event.failed_controls).map((link) => record(link.control)).filter((control) => control.id)
...
function searchableText(event: Record<string, unknown>) {
  const risk = record(event.risk)
  const failedControls = arrayRecords(event.failed_controls).map((link) => record(link.control)).flatMap((control) => [control.kode, control.nama])
  return [event.kode, event.unit_nama, event.nama_peristiwa, event.kronologi, event.status, event.klasifikasi_limit, event.jenis_dampak, event.uraian_dampak, event.akar_masalah, event.kegagalan_kontrol, ...failedControls, risk.kode, risk.kategori, risk.peristiwa].map((value) => String(value || '').toLocaleLowerCase('id-ID')).join(' ')
}
