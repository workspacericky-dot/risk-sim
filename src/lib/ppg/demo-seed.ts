...
export async function ensurePpgDemoData(actorId?: string): Promise<DemoSeedResult> {
  ...
  const [existingRisks, existingReports, existingPrograms, existingLossEvents, existingAppetites, existingAssignments] = await Promise.all([
    ...
    db.from('ppg_satker_program_assignments').select('id', { count: 'exact', head: true }),
  ])
  if (existingRisks.count === 5 && existingReports.count === 369 && existingPrograms.count === 1 && (existingLossEvents.count || 0) >= 50 && (existingAppetites.count || 0) >= 25 && (existingAssignments.count || 0) >= 4) return { ok: true, created: false, message: 'Data simulasi sudah tersedia.' }
  ...
  result = await db.from('ppg_programs').insert({ id: ids.program, kode: 'SIM-PPG-2026-01', nama: 'Penguatan Pengendalian Gratifikasi Periode Rawan', snapshot_id: ids.snapshot, analysis_start: '2026-01-01', analysis_end: '2026-06-30', program_start: '2026-07-01', program_end: '2026-08-31', period_label: 'Semester II 2026', status: 'selesai', ditetapkan_by: actorId || null, ditetapkan_at: '2026-06-25T08:00:00Z', catatan_penetapan: 'Program dummy telah direview dan ditetapkan UPG Pusat untuk seluruh klaster simulasi.', created_by: actorId || null })
  ...
  const satkerAssignments = registers.slice(0, 4).map((register, index) => ({
    id: demoUuid('7', index + 1), program_item_id: ids.programItem, register_id: register.id, unit_kerja_id: register.unit_kerja_id,
    planned_start: `2026-07-${String(index + 1).padStart(2, '0')}`, planned_end: '2026-08-31', pic_jabatan: index % 2 ? 'Panitera' : 'Ketua UPG Satker', planning_notes: 'Rencana dummy: koordinasi pelaksana, penyiapan materi, dan dokumentasi evidence.', planned_by: actorId || null, planned_at: '2026-06-28T08:00:00Z',
    actual_end: index < 3 ? `2026-08-${String(27 + index).padStart(2, '0')}` : null, realization_summary: index < 3 ? 'Program terlaksana melalui briefing terarah, simulasi penolakan, dan penguatan media pengendalian pada area layanan.' : '', efektivitas_program: index < 3 ? (index === 2 ? 'cukup_efektif' : 'efektif') : null, evidence_url: index < 3 ? `https://example.invalid/simulasi/program/satker-${index + 1}` : '', kemungkinan_treated: index < 3 ? 1 : null, dampak_treated: index < 3 ? 3 : null, skor_treated: index < 3 ? 3 : null, level_treated: index < 3 ? 'Rendah' : null, post_submitted_by: index < 3 ? actorId || null : null, post_submitted_at: index < 3 ? '2026-09-01T08:00:00Z' : null, validation_status: index === 0 ? 'disetujui' : index < 3 ? 'menunggu' : 'belum_diajukan', validation_notes: index === 0 ? 'Realisasi dan evidence dinilai memadai.' : '', validated_by: index === 0 ? actorId || null : null, validated_at: index === 0 ? '2026-09-02T08:00:00Z' : null,
  }))
  result = await db.from('ppg_satker_program_assignments').insert(satkerAssignments)
  if (result.error) return failed('Tahapan program Satker', result.error.message)
...
export async function resetPpgDemoData(actorId?: string): Promise<DemoSeedResult> {
  const db = createPpgAdminClient(PPG_DEMO_SCENARIO_ID)
  const order = ['ppg_ai_action_candidates', 'ppg_ai_recommendation_runs', 'ppg_satker_program_assignments', 'ppg_programs', 'ppg_loss_events', 'ppg_loss_event_code_counters', 'ppg_risk_import_batches', 'ppg_risk_candidates', 'ppg_import_batches', 'ppg_register', 'ppg_risk_appetites', 'ppg_audit_log', 'ppg_analysis_snapshots', 'ppg_risk_library', 'ppg_control_library']
